...
function Cart() {
  ...
  const { value, setValue, handleChange, makeOrder, handleCollect, bag, handleCloseBag, normalorderP, handleOpenNormalOP, cartopen, handleCartclose, payp4b, handleOpenPayp4b } = useStateContext();
          ...
          <TabContext value={value}>
            <div style={{backgroundColor: "rgba(255, 255, 255, 0.9)", position: "sticky", top: 0, zIndex: "10"}} className={classes.reviewTopTab}>
              ...
              <Tabs classes={{root:classes.hmStyle, indicator:classes.ndicateThick }} sx={{"& .MuiTab-root.Mui-selected": {color:"black"}, position:"sticky" ,top: 45, zIndex: 15, minHeight: 40}} value={value} centered onChange={handleChange}>
                <Tab label={"My Cart".concat(" " , cartItems.reduce((a, c) => a + c.quantity, 0) )} value="Cart" />
                <Tab label="My Bag" value="Bag"/>
              </Tabs>
            </div>
            <TabPanel value="Cart" style={{padding: 0, height: '90%'}} >
            ...
            <TabPanel value="Bag" style={{padding: 0}}>
                ...
                {bagitems.length === 0 && 
                  <div className=" w-full h-full flex justify-center place-items-center">
                    <div className="empty-cart">
                      <div className="cart-icn block">  
                        <Image 
                          src="https://res.cloudinary.com/dddx5qpji/image/upload/v1679908684/noun-shopping-bag-1399979__2_-removebg-preview_bjhsrx.png"
                          width={400}
                          height={400}
                          alt=""
                        />
                ...
                {bagitems.map((bag) => (
                  <div key={bag._id} className='grid grid-cols-2 gap-col-4 gap-y-3 md:grid-cols-3 lg:grid-cols-4'>
                    {bag.orderItems?.map((images) => (
                      <div key={images}>
                        <Image alt="" className="bg-gray-100" width={364} height={484} src={images.image[0].item}/>
                      </div>
                    ))}
                  </div>
                ))}
